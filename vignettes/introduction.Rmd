...
df_sim <- r_data_frame(
    ...
    height_cm,
    group(x = c("Yes", "No"), prob = c(0.7, 0.3), name = "CABG"),
    group(x = c("Yes", "No"), prob = c(0.3, 0.7), name = "AKE")
)
df_sim

df_sim %>% do(
  calc_sts(proc_valve = .$AKE, proc_cabg = .$CABG, age = .$Age, gender = .$Sex, crea = .$crea, lvef = .$lvef)
)
```
